...
class Client(QtGui.QWidget):
    ...
    def populate(self):
        ...
        self.parameterBox.setText(
            ...
            "x0, y0 = 500, 600\n"
            "x, y = np.meshgrid(range(1040), range(1392))\n"
            "r2 = (x - x0)**2 + (y - y0)**2\n"
            "cloud = ((x - x0)**2 <= 500**2) & ((y - y0)**2  <= 500**2)\n"
...
#        self.outputBox.setFontPointSize(32)
        ...
        self.setWindowTitle("Pixelfly Viewer")
    ...
    def connect(self):
        self.cxn = connection()
        yield self.cxn.connect(name='pixelfly viewer')
        server = yield self.cxn.get_server('yesr13_pixelfly')
    ...
    def receive_update(self, c, update):
        print(update)
        self.pathBox.setText("Q:\\data\\20211223\\pixelfly.hdf5")
        self.displayImage()

    def getImagePath(self):
        return DATADIR + "20211223\\pixelfly.hdf5"
...
#        h5f.close()
        #h5f = h5py.File(os.path.join(DATADIR, "20211223/pixelfly_darkframe.hdf5"), "r")
        #ref_image = np.array(h5f['image'], dtype='float')
        #ref_bright = np.array(h5f['bright'], dtype='float')
        #h5f.close()
        ...
        ref_image = ref_bright = np.array(h5f['dark'], dtype='float') 
...
#        h5f.close()
    ...
    def displayImage(self):
        image_path = "Q:\\data\\20211223\\pixelfly.hdf5"
