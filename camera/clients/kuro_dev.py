...
class Client(QtGui.QWidget):
    ...
    def populate(self):
        ...
        self.setWindowTitle("Kuro Viewer")
...
#            ttf = (6 * f3(-popt_fermi[-1]))**(-1 / 3)
        ...
        if fit:
            ...
            ax2.plot(np.sum(image[zoom].reshape((101, 101)), axis = 0), color = 'black')
            ax2.plot(np.sum(fitted_cloud_imshow, axis = 0), '--', color = 'red', alpha = .5)
           # ax2.set_xlim(xmin, xmax)
            ax3 = self.figure.add_subplot(224)
            ax3.plot(np.sum(image[zoom].reshape((101, 101)), axis = 1), color = 'black')
