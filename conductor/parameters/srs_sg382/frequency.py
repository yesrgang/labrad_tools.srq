    ...
    def update(self):
        ...
        try:
            if self.value is not None:
                ...
                amp_LO = self.server.parameters.get("srs_sg382.amplitude")
                if amp_LO is None:
                    amp_LO.value = 10
                print("SRS SG382 output: {} MHz, {} dBm.".format(self.value/1e6, amp_LO.value))
                #print(type(amp_LO.value))
                self.cxn.yesr14_srs.set_wf_sine_full(self.value, amp_LO.value, 5)
